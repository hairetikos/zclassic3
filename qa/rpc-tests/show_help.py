...
from test_framework.util import assert_equal, zclassicd_binary
...
Usage:
  zclassicd [options]                     Start Zcash Daemon
...
|
  ...
  -pid=<file>
       ...
       datadir location. (default: zclassicd.pid)
...
Wallet options:
  ...
  -walletrequirebackup=<bool>
       ...
       confirmed with the `zclassicd-wallet-tool` utility. A user may start zclassicd
...
|       generated by a zclassicd version before v5.4.0
|
...
class ShowHelpTest(BitcoinTestFramework):
    ...
    def show_help(self, expected, extra_args):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stdout:
            args = [ zclassicd_binary(), "--help" ] + extra_args
