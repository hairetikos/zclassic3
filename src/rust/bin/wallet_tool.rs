...
struct CliOptions {
    ...
    #[options(
        no_short,
        help = "Specify configuration filename, relative to the data directory (default: zclassic.conf)",
        meta = "FILENAME"
    )]
    conf: Option<String>,
...
enum WalletToolError {
    #[error("zclassic-cli executable not found")]
    ZcashCliNotFound,

    #[error("Unexpected response from zclassic-cli or zclassicd")]
    UnexpectedResponse,

    #[error("Could not connect to zclassicd")]
    ZcashdConnection,

    #[error("zclassicd -exportdir option not set")]
    ExportDirNotSet,
...
fn run(opts: &CliOptions) -> anyhow::Result<()> {
    ...
    println!(concat!(
        ...
        "zclassicd wallet is backed up reliably.\n\n",
        "  👛 ➜ 🗃️ \n"
    ));

    println!("Checking that we can connect to zclassicd...");
    let zcash_cli = zcash_cli_path()?;

    // Pass an invalid filename, "\x01", and use the error message to distinguish
    // whether zclassicd is running with the -exportdir option, running without that
    ...
    let cli_err: Vec<_> = from_utf8(&out.stderr)
        .with_context(|| "Output from zclassic-cli was not UTF-8")?
        .lines()
    ...
    if !cli_err.is_empty() {
        if cli_err[0].starts_with("Error reading configuration file") {
            println!(
                "\nNo, we could not read the zclassicd configuration file, expected to be at\n{:?}.",
                Path::new(opts.datadir.as_ref().map_or("~/.zclassic", |s| &s[..])).join(Path::new(
                    opts.conf.as_ref().map_or("zclassic.conf", |s| &s[..])
                )),
            );
            println!(concat!(
        ...
        if cli_err[0].starts_with("error: couldn't connect") {
            println!(concat!(
                "\nNo, we could not connect. zclassicd might not be running; in that case\n",
                ...
                "(Don't forget to restart zclassicd without '-exportdir' after finishing\n",
                "the backup, if running it long-term with that option is not desired\n",
                "or would be a security hazard in your environment.)\n\n",
                "If you believe zclassicd is running, it might be using an unexpected port,\n",
                "address, or authentication options for the RPC interface, for example.\n",
                "In that case try to connect to it using zclassic-cli, and if successful,\n",
                "use the same connection options for zclassicd-wallet-tool (see '--help' for\n",
                "accepted options) as for zclassic-cli.\n"
            ));
            return Err(WalletToolError::ZcashdConnection.into());
        }
        if cli_err[0] == "error code: -28" {
            println!(concat!(
                "\nNo, we could not connect. zclassicd seems to be initializing; please try\n",
    ...
    const REMINDER_MSG: &str = concat!(
        "\n\nPlease start or restart zclassicd with '-exportdir' set to the absolute\n",
        "path of the directory you want to save the wallet export file to.\n",
        "(Don't forget to restart zclassicd without '-exportdir' after finishing\n",
        "the backup, if running it long-term with that option is not desired\n",
        "or would be a security hazard in your environment.)\n",
    ...
    if cli_err.len() >= 3
        ...
        && cli_err[2].contains("zclassicd -exportdir")
    {
        println!(
            "\nIt looks like zclassicd is running without the '-exportdir' option.{}",
            REMINDER_MSG
        );
    ...
    {
        println!(
            "\nThere was an unexpected response from zclassic-cli or zclassicd:\n> {}{}",
            cli_err.join("\n> "),
            REMINDER_MSG,
        );
    ...
    let out = loop {
            ...
            && cli_err[2].contains("overwrite existing")
        {
            println!(concat!(
                ...
                "directory specified by the '-exportdir' option to zclassicd."
            ));
            continue;
        } else {
            break out;
    ...
    exec(&zcash_cli, &cli_args, Some(phrase))
        .and_then(|out| {
            ...
            if !cli_err.is_empty() {
                if cli_err[0].starts_with("error: couldn't connect") {
                    println!("\nWe could not connect to zclassicd; it may have exited.");
                    return Err(WalletToolError::ZcashdConnection.into());
                } else {
                    println!(
                        "\nThere was an unexpected response from zclassic-cli or zclassicd:\n> {}",
            ...
            println!(concat!(
                "\nThe backup of the emergency recovery phrase for the zclassicd\n",
                "wallet has been successfully confirmed 🙂. You can now use the\n",
                "zclassicd RPC methods that create keys and addresses in that wallet.\n",
                "\n",
                "The zclassicd wallet might also contain keys that are *not* derived\n",
        ...
        .inspect_err(|_| {
            println!(concat!(
                "\nzclassic-wallet-tool was unable to communicate to zclassicd that the\n",
                "backup was confirmed. This can happen if zclassicd stopped, in which\n",
                "case you should try again. If zclassicd is still running, please seek\n",
                "help or try to use 'zclassic-cli -stdin walletconfirmbackup' manually.\n"
            ));
        })?;
    Ok(())
}
...
fn zcash_cli_path() -> anyhow::Result<PathBuf> {
    ...
    exe.set_file_name("zclassic-cli");
    exe.set_extension(EXE_EXTENSION);

    debug!("Testing for zclassic-cli at {:?}", exe);
    if exe.exists() {
        return Ok(exe);
    }
    ...
    match stdin {
        ...
        Some(data) => {
            ...
            cli_process
                .stdin
                .take()
                .with_context(|| "Could not open pipe to zclassic-cli's stdin")
                .and_then(|mut pipe: ChildStdin| -> anyhow::Result<()> {
                    pipe.write_all(data.as_bytes())?;
                    pipe.write_all("\n".as_bytes())?;
                    Ok(())
                })
                .with_context(|| "Could not write to zclassic-cli's stdin")?;
...
fn clear_and_show_cautions(export_path: &str) {
    ...
    println!(
        concat!(
            "\nIMPORTANT: Secrets that allow spending all zclassicd wallet funds\n",
            "have been left in the file '{}'.\n\n",
            "Don't forget to restart zclassicd without '-exportdir', if running it\n",
